...
class StringCalculator{
  ...
  add(str){
    ...
    if(typeof str != "string"){
      ...
      if (this.hasInvalidFormat(str)) {  // check if input string format is valid 
  ...
  hasInvalidFormat(numbers) {
    ...
    const invalidPattern2 = /[\/|\/\/]$/;  // Starts with / or //
    const misplacedDelimiters = /,\n|\n,/;  // Misplaced delimiter combinations
    if (numbers.startsWith('/') || numbers.startsWith('//')) {
      return true;
    }
    return invalidPattern1.test(numbers) || invalidPattern2.test(numbers) || misplacedDelimiters.test(numbers);
  }
